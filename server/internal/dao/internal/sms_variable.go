...
type SmsVariableColumns struct {
	Id          string //
	Title       string // 变量标题
	Name        string // 变量名（如 usermobile）
	SourceType  string // 来源类型：1=字段提取 2=SQL查询 3=内置Helper
	SqlQuery    string // SQL查询语句（source_type=2 时）
	MethodName  string // Helper方法路径（source_type=3 时）
